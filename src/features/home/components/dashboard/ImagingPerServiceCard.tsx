...
import { imagingPerServiceDay, imagingPerServiceMonth } from "@/fakedata/doctorStatsData.ts";
import { COLORS_DASHBOARD } from "@/constants/dashboard/colors";

export default function ImagingPerServiceCard() {
    const [mode, setMode] = useState<"day" | "month">("day");
    const data = mode === "day" ? imagingPerServiceDay : imagingPerServiceMonth;

    return (
        <Card>
            <CardHeader className="flex justify-between items-center">
                <CardTitle>
                    Số ca theo dịch vụ chụp chiếu {mode === "day" ? "(trong ngày)" : "(trong tháng)"}
                </CardTitle>
